...
### intersect と render の境界

どちらもドメイン層だが、責務の粒度が違う。

- intersect/: **幾何の原子**。1本の ray と 1つの形状の交差判定だけを扱う。
    - 返すのは最小正の `t` と法線（`t_hit` を埋めるための情報）
    - 光や影の判断は行わない
- render/: **交差の利用側**。primary ray / shadow ray を生成し、交差結果を使って色を決める。
    - どの ray を飛ばすか、交差をどう使うか（ambient + diffuse + shadow）を決める

言い換えると intersect は「交差判定を提供する側」、render は「交差判定を使う側」。
